    ...
    def build(self, cls: type):
        ...
        for member_key, member_value in members.items():
            ...
            if member_key.startswith('_'):
                raise SettingsError(ErrorCodes.SETTING_KEY_IS_REQUIRED, key=member_key)
            ...
            if cast not in ALLOWED_BUILTIN_TYPES:
                raise SettingsError(ErrorCodes.SETTING_TYPE_IS_NOT_ALLOWED, key=member_key)
            setting_value = self.parser.get_setting(member_key, cast=cast, default=member_value.default)
